...
class Categories extends MY_Controller
{
    ...
    {
        $this->dataModule['results'] =  $this->Sitefunction->get_all_rows(TBL_CATEGORIES, "*", array('status!=' => 9), array(), array(), '', '', array(), '', array(), array());
        $this->load->view('categories/index', $this->dataModule);
    }
    public function add()
    {
        ...
        if ($postData && !empty($postData)) {
            ...
            if ($this->form_validation->run() != FALSE) {
                ...
                if ($this->Sitefunction->insert(TBL_CATEGORIES, $data_array)) {
        ...
        $this->load->view('categories/add', $this->dataModule);
    }
    public function delete()
    {

        $this->Sitefunction->delete(TBL_CATEGORIES, array('id' => $this->input->get_post('id')));
    }
    ...
    public function multiple_delete()
    {
        ...
        foreach ($ids as $id) {
            $this->Sitefunction->delete(TBL_CATEGORIES, array('id' => $id));
        }
    }
    public function edit($id)
    {
        $this->dataModule['results'] = $this->Sitefunction->get_single_row(TBL_CATEGORIES, '*', array('id' => $id));
        ...
        if ($postData && !empty($postData)) {
            ...
            if ($this->form_validation->run() != FALSE) {
                ...
                if ($this->Sitefunction->update(TBL_CATEGORIES, $data_array, array('id' => $id))) {
        ...
        $this->load->view('categories/edit', $this->dataModule);
    }
}
?>