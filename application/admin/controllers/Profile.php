...
class Profile extends MY_Controller {
	...
	public function index() {
        $this->dataModule['results']= $this->Sitefunction->get_single_row(TBL_ADMIN, '*', array('id'=>$this->user_id));
			...
			if ($this->form_validation->run() != FALSE)
			{
                ...
                if(isset($_FILES['image']) && $_FILES['image'] != '')
                {
                        ...
                        if ($_FILES["image"]["error"] > 0) { 
                            ...
                            move_uploaded_file($_FILES['image']['tmp_name'], UPLOAD_PATH.'profile/'. $p_profile_image1);
                ...
                if($this->Sitefunction->update(TBL_ADMIN, $data_array, array('id'=>$this->user_id))){
    ...
    public function change_password(){
        ...
        if($postData && !empty($postData)) {
			...
			if ($this->form_validation->run() != FALSE)
			{
                ...
                $check_password= $this->Sitefunction->get_single_row(TBL_ADMIN, '*', array('password'=>md5($old_password)));
