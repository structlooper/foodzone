	...
	public function index() {
		...
		if(!empty($post)) {
			...
			if ($this->form_validation->run() != FALSE)
			{
				...
				$admin_find= $this->Sitefunction->get_single_row(TBL_OWNERS, '*', array('email'=>urlencode($username), 'password'=>md5($password)));
				if(!empty($admin_find)) {
					$data= array('owner_id'=>$admin_find->id);
	...
	public function forgot_password() {
		...
		if(!empty($post)) {
			...
			if ($this->form_validation->run() != FALSE)
			{
				...
				$admin_find= $this->Sitefunction->get_single_row(TBL_OWNERS, '*', array('email'=>urlencode($this->input->post('email'))));
				if(!empty($admin_find)) {
